...
function removeFirstOccurrences(str, value) {
  let o;
  const c = str.indexOf(value);
  const y = value.length + c;
  if (str.includes(value)) {
    o = str.substring(0, c) + str.substring(y);
  } else {
    o = str;
  }
  return o;
...
function isPalindrome(str) {
  ...
  const str2 = str.split('').reverse().join('').toLowerCase();
  if (str1.toLowerCase() === str2.replace(/[\s,?!]/g, '')) {
    s = true;
  } else {
    s = false;
  }
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, function rment(letter) {
    return rot13[alphabet.indexOf(letter)];
  });
}
